...
import {
  ...
  AlertCircle, Loader2, RefreshCw, MapPin, Users, Shield,
} from 'lucide-react'
...
function stationStatusVariant(status: string) {
...
export function OperatorProfile() {
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const [meData, stations] = await Promise.all([
        api.get<{ user: UserType }>('/auth/me'),
        api.get<StationDetail[]>('/stations'),
      ])
      ...
      const mine = (stations ?? []).find(
        (s) => (s.operatorId as any)?._id === me._id || s.operatorId === (me._id as any)
      )
      setStation(mine ?? null)
    } catch (err) {
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-start justify-between gap-4 mb-4">
                ...
                <Badge variant={stationStatusVariant(station.status)} className="capitalize shrink-0">
                  {station.status}
                </Badge>
              </div>
              ...
              {station.assignedTechnicians && station.assignedTechnicians.length > 0 && (
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {station.assignedTechnicians.map((tech) => (
                      <div key={tech._id} className="flex items-center gap-2 px-3 py-1.5 bg-white border rounded-lg text-sm">
                        <div className="w-6 h-6 bg-primary/10 rounded-full flex items-center justify-center">
                          <User className="w-3.5 h-3.5 text-primary" />
                        </div>
                        <span className="font-medium">{tech.fullName}</span>
                        {tech.phone && <span className="text-gray-400 text-xs">{tech.phone}</span>}
                      </div>
                    ))}
                  </div>
                </div>
